...
struct MechanismVisualizer{M <: Mechanism}
	mechanism::M
	...
	function MechanismVisualizer(mechanism::M, 
		...
		_setmechanism!(mvis, frame_to_visuals)
		mvis
	end
end
...
function _setmechanism!(mvis::MechanismVisualizer, frame_to_visuals)
	mechanism = mvis.mechanism
	vis = mvis.visualizer
...
function setstate!(mvis::MechanismVisualizer, state::MechanismState)
	@assert mvis.mechanism === state.mechanism
